...
function avatarGuide() {
  const shape = shapeById(state.aspect),
    av = shape.avatar;
  if (!av || !state.avatarGuide) return null;
  const f = frame(),
    k = f.w / shape.png; /* banner pixels → canvas units */
  const r = av.r * k,
    ring = av.ring * k;
  ...
  clip.appendChild(el("circle", { cx, cy, r: r * 0.97 }));
  g.appendChild(clip);
  g.appendChild(
    el("path", {
      d: `M${cx - r * 0.56} ${cy + r * 0.58} a${r * 0.56} ${r * 0.5} 0 0 1 ${r * 1.12} 0 v${r * 0.5} h${-r * 1.12} z`,
      fill: figure,
