...
export function bandEditor(hooks) {
  ...
  col.value = state.accent;
  const hex = input("text", "hex wide");
  hex.value = state.accent;
  ...
  hex.onchange = () => {
    if (isHex(hex.value)) apply(hex.value.toLowerCase());
    else hex.value = state.accent;
  };

  const grow = div("grow");
  grow.append(col, hex, lbl);
