...
export function cycleAxis(cycles, phase, mode) {
  ...
  const L = ax.len / Math.max(cycles, 0.05);
  const ux = (ax.x2 - ax.x1) / ax.len,
    uy = (ax.y2 - ax.y1) / ax.len;
...
export function buildSVG() {
  ...
  if (st.mode === "solid") {
    ...
    g.dataset.cyc = st.holo.cycles;
    g.dataset.kind = "reflect";
    defs.appendChild(g);
    paint.appendChild(el("rect", Object.assign({ fill: `url(#${P}g)` }, cover)));
    if (st.holo.sheen > 0) {
      ...
      const sh = linearGrad(
        ...
        [
          { o: 0, c: "#ffffff", a: 0 },
          { o: 0.42, c: "#ffffff", a: 0 },
          { o: 0.5, c: "#ffffff", a: +(st.holo.sheen * 0.85).toFixed(3) },
          { o: 0.58, c: "#ffffff", a: 0 },
          { o: 1, c: "#ffffff", a: 0 },
        ],
        sax,
      ...
      sh.dataset.cyc = st.holo.cycles * 0.42;
      sh.dataset.kind = "reflect";
      sh.dataset.speed = "-1.7";
      defs.appendChild(sh);
